...
int Block::load_miner_tx(ifstream & in)
{
    ...
    // (1) Check the version flag
    // (2) Skip unlock varint
    // (3) Check that there is only one input transaction
    // (4) Check format of input transaction (0xff = tf_gen)
    // (5) Skip input transaction (one varint indicating height of the block)
    if(tools::expect_byte(in, 0x01) < 0     or
       tools::skip_varint(in) < 0           or
