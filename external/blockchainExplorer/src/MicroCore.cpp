...
bool MicroCore::get_block_blob_by_height(const uint64_t& height, cryptonote::blobdata& data)
{
    try
    {
        data = m_blockchain_storage.get_db().get_block_blob_from_height(height);
    } catch (...)
    {
        cerr << "Something went terribly wrong when getting block " << height
             << endl;
...
bool MicroCore::get_block_blob_by_hash(crypto::hash &h, cryptonote::blobdata& data)
{
    try
    {
        data = m_blockchain_storage.get_db().get_block_blob(h);
    } catch (...)
    {
        cerr << "Something went terribly wrong when getting block by hash..." << h.data
             << endl;
